	...
	if( defined('form_pass') ) {
		...
		if( form_pass === 'pass' ) {
			...
			if( $action === 'update' ) {
				...
				header('Location: '. site_host .'/store/?notification=category-updated^'. $id);

			}
			else {

				$model::erase('store_categories', [

					'criterion' => 'id::'. $id

				]);

				header('Location: '. site_host .'/store/?notification=category-erased^'. $id);
			...
			header('Location: '. site_host .'/store/category/'. $id .'/edit/?notification=no-changes');
